...
fields={"curX":"N/A", "curY":"N/A", "curZ":"N/A",
	"incSmall":10, "incLarge":100}
...
def refresh(event):
	for i,k in enumerate(["curX","curY","curZ"]):
		if i>=len(stages):
			continue
		v=np.round(stages[i].position*factor,2)
...
def gotoXYZ(event):
	for i,k in enumerate(["curX","curY","curZ"]):
		...
		stages[i].setPosition(v/factor)

def chg(whichStage,bigOrSmall,plusOrMinus=1):
	by={"big":float(fields["incLarge"].get()), "small":float(fields["incSmall"].get())}[bigOrSmall]
	print(by)
	if whichStage>=len(stages):
		return
	stages[whichStage].changePosition(by*plusOrMinus/factor)
...
def focusOnSample(event):
	stages[2].changePosition(-25/factor)
